...
```bash
pip install agent-shim
# or, with MLLANG signal extraction:
pip install 'agent-shim[mllang]'
```
...
`agent-shim` works just fine on plain LLM responses. Drop the `parser="mllang"` and you still get:

- Token in/out counts
- Latency
- Cost estimate ($5/M tokens default; override via `cost_per_M=` or `AGENT_SHIM_COST_PER_M` env)
- Model distribution
- p50 / p99 latency
...
When `parser="mllang"` is set and `agent-shim[mllang]` (or `mllang-protocol`) is installed, agent-shim looks at each response for an MLLANG packet (either as a bare line or inside a ```mllang fenced block). When found, it extracts:
...
from agent_shim import instrument
...
from agent_shim import observe
...
|----------|---------|------|
| `AGENT_SHIM_LOG` | `agent_shim.jsonl` | Default log path when no path is passed |
| `AGENT_SHIM_COST_PER_M` | `5.0` | USD per 1M tokens for cost estimate |

## Privacy posture

- agent-shim does not phone home. No HTTP egress. Logs are local JSONL.
